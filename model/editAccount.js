      ...
      res.json({"message":"err.. no params session rec"});
        ...
        if(usernameType==null || usernameType==undefined || usernameType==""){
          ...
          if(passwordType==null || passwordType==undefined || passwordType==""){
            ...
            var query="select id_user from `session` where session_code='"+session+"'";
            connection.query(query,function(err,rows){
              if(err){
                res.json({"message":"err.. error in selectiong session","query":query});
              }else{
                if(rows.length>0){
                  var idUserz = rows[0].id_user;
                  connection.query("select username,password,id_role from `user` where id_user="+idUserz,function(err,rows){
                    if(err){
                      ...
                      if(rows.length>0){
                        ...
                        connection.query("select role_name from `role` where id_role="+idRole,function(err,rows){
                          if(err){
                            res.json({"messgae":"err.. error on selecting role"});
                          }else{
                            if(rows.length>0){

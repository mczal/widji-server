            ...
            if(rows.length>0){
              if(rows.length==1){
                ...
                if(rows[0].id_role == 2){
                  ...
                  connection.query("select name,jumlah_bayar from `order` where no_bon='"+no_bon+"'",function(err,rows){
                    if(err){
                      ...
                      if(rows.length>0){
                        ...
                        connection.query(q,function(err,rows){
                          if(err){
                            ...
                            res.json({"message":"success selecting","error":"success","order_name":orderName,"no_bon":no_bon,"jumlah_bayar":jumlahBayar,"count":rows.length,"content":rows});
                          }
                        });
                      }else{

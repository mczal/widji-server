          ...
          if(err){
            ...
            if(rows.length>0){
              if(rows.length==1){
                ...
                if(rows[0].id_role == 2){
                  var q = "select product_category.name,product.media,product.size,product.status,product.weight,product.imgbase64,order_item.quantity from `order` join `order_item` on order.id=order_item.order_id join `product` on order_item.product_id=product.id join `product_category` on product.category_id=product_category.id where order.no_bon = '"+no_bon+"'";

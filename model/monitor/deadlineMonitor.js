...
deadlineMonitor.prototype.handleRoutes = function(router,connection){
  router.post('/deadlineMonitor',function(req,res){
    connection.query("select no_bon,name,jumlah_bayar,customer_id,created_at,tanggal_pengambilan,jam_pengambilan,datediff(tanggal_pengambilan,curDate()) as selisih,keterangan,status from `order`",function(err,rows){
      if(err){

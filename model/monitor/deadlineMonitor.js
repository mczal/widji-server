...
deadlineMonitor.prototype.handleRoutes = function(router,connection){
  router.post('/deadlineMonitor',function(req,res){
    var query = "select no_bon,name,jumlah_bayar,customer_id,created_at,tanggal_pengambilan,jam_pengambilan,datediff(tanggal_pengambilan,curDate()) as selisih,keterangan,status_pengerjaan,worker,laci from `order`  where (datediff(tanggal_pengambilan,curDate())) >= -10 AND (datediff(tanggal_pengambilan,curDate())) <=14 order by selisih";

                ...
                connection.query("select id_user from `user` where username='"+username+"'",function(err,rows){
                  if(err){
                    ...
                    if(rows.length>0){
                      ...
                      connection.query("select id_counter from `counter` where ip_addrs='"+ipCounter+"' and id_user="+idUser,function(err,rows){
                        if(err){
                      ...
                      res.json({"message":"err.. no rows with given username"});
                    }
                  }
                });
              }else{
